...
public class CLController {
    ...
    @Command(
            ...
            aliases = ("ar"),
            description = "Add relationship between classes")
        ...
        try {
            ...
            if (!model.relationshipExist(source, dest)) {
                ...
                if (type != null && editor.addRelationship(source, dest, type)) {
                    view.show(
                            type
                                    + " Relationship successfully created from "
                                    + source
                                    + " to "
                                    + dest);
                } else {
    ...
    /** Enables the editing of existing relationships from the CLI editor Loops until the */
        ...
        try {
            ...
            if (model.relationshipExist(source, dest)) {
                ...
                switch (field) {
                    ...
                    default:
                        view.show(
                                "Unfortunately, we don't support changing the "
                                        + field
    ...
    /** Gets class, method, and parameters from user and returns if action succeeded or not */
            ...
            while (!empty_input) {
                if (paramName.equalsIgnoreCase("stop") || paramName.equals("")) {
                    ...
                    if (exist) {
                        ...
                        if (type.isEmpty()) {
                            // Will restart loop which will immediately come back to this point b/c
                            // paramName is still stored
    ...
    private void CL_editMethod() {
        try {
            ...
            view.show(
                    "What part of "
                            + editMethod.getName()
                            + " do you want to change? (Name or Type)");
            input = sc.nextLine().replaceAll("\\s", "");
            while (true) {
                if (input.equalsIgnoreCase("name")) {
                    // Edit name
    ...
    private void CL_removeParam() {
        try {
            view.show(model.listClassNames());
            view.show(
                    "What class does the method you want to remove the parameter from belong to?");
            String className = sc.nextLine();
            activeClass = model.fetchClass(className);
            view.show(model.listMethods(activeClass));
            view.show("Enter the number of the method you want to rename('stop' to cancel)");
    ...
    @Command(
            ...
            aliases = ("rap"),
            description = "Removes all parameters")
        ...
        try {
            ...
            while (loop) {
                ...
                if (paramName.equalsIgnoreCase("stop") || paramName.equals("")) {
                    ...
                    if (!input.equalsIgnoreCase("all")) {
                        // Only need to check if new paramName is in method if all parameters are
                        // not
                        // being replaced
            ...
            if (changeAll) {
                ...
                view.show(
                        "Parameter "
                                + oldParam.getName()
                                + " was replaced with new parameter list.");
            }
        } catch (Exception e) {
            view.show(e.getMessage());
        }
