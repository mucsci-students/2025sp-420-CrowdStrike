...
public class FileManager {
    ...
    public UMLModel load(String path) throws Exception {
        ...
        if (!modelJson.isJsonObject()) throw new InvalidObjectException("given json is invalid");
    ...
    private void addRelationships(JsonElement json, UMLModel model) throws Exception {
        ...
        if (!json.isJsonObject()) throw new InvalidObjectException("relationship is not an object");

        relationshipObject = json.getAsJsonObject();
        if (!(relationshipObject.has("source")
                && relationshipObject.has("destination")
        ...
        e.addRelationship(
                relationshipObject.get("source").getAsString(),
                relationshipObject.get("destination").getAsString(),
                relationshipType);
    }
    ...
    private void addFields(JsonArray json, ArrayList<AttributeInterface> fields) throws Exception {
        for (JsonElement field : json) {
            if (!field.isJsonObject()) throw new InvalidObjectException("Field is not an object");
            ...
            fields.add(
                    new Field(
                            ...
                            field.getAsJsonObject().get("type").getAsString()));
        }
    }

    private void addMethods(JsonArray json, ArrayList<AttributeInterface> methods)
            throws Exception {
        for (JsonElement methodJson : json) {
            ...
            method =
                    new Method(
                            ...
                            methodObject.get("return_type").getAsString(),
                            parameterList);
            methods.add(method);
        }
    }

    private void addParameters(JsonArray json, ArrayList<Parameter> parameterList)
            throws Exception {
        for (JsonElement parameter : json) {
            if (!parameter.isJsonObject()
                    && parameter.getAsJsonObject().has("name")
                    && parameter.getAsJsonObject().has("type"))
                throw new InvalidObjectException("Parameter is malformed");
            parameterList.add(
                    new Parameter(
                            parameter.getAsJsonObject().get("name").getAsString(),
                            parameter.getAsJsonObject().get("type").getAsString()));
        }
    }
    ...
    private <T> String jsonCommas(
            String begin, String end, ArrayList<T> data, Function<T, String> convert) {
    ...
    private String classToJson(ClassObject classobj) {
        ...
        ret.append(
                jsonCommas("\"methods\": [", "],", classobj.getMethodList(), this::MethodsToJson));
    ...
    private String FieldsToJson(AttributeInterface field) {
        ...
        return String.format(
                "{ \"name\": \"%s\", \"type\": \"%s\"}", fld.getName(), fld.getVarType());
    }
    ...
    private String ParamToJson(Parameter param) {
        return String.format(
                "{ \"name\": \"%s\", \"type\": \"%s\"}", param.getName(), param.getType());
    }

    private String relationshipToJson(Relationship relationship) {
        return String.format(
                "{\"source\": \"%s\",\"destination\": \"%s\",\"type\":\"%s\"}",
