...
public class Method implements AttributeInterface {
    ...
    public boolean equals(Method compareMethod) {
        ...
        if (this.name.equals(compareMethod.getName())
                && this.paramList.size() == compareMethod.getParamList().size()) {
            // Check types of parameters in both methods to make sure they are the same (in type and
            // order)
            int index = 0;
            for (Parameter param : paramList) {
                if (!param.getType().equalsIgnoreCase(paramList.get(index).getType())) {
                    // If parameters in the same position don't have the same type, they are not
                    // equal
                    return false;
                }
                index++;
