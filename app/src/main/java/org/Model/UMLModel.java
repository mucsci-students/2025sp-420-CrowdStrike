...
public class UMLModel implements UMLModelInterface {
    ...
    private UMLModel(UMLModel m) {
        ...
        for (ClassObject o : m.getClassList()) classList.add(new ClassObject(o));
     ...
     * @return ClassObject with specified name if it exists returns null if class does not exist
     * @throws Exception
     */
    ...
    public String listClassInfo(ClassObject cls) {
        ...
        for (AttributeInterface mthd : cls.getMethodList()) {
            ...
            finalString =
                    finalString
                            + "   "
                            + activeMethod.getName()
                            + listParams(activeMethod)
                            + " -> "
                            + activeMethod.getReturnType()
                            + "\n";
        }
        ...
        while (index < relationshipList.size()) {
            ...
            if (curRelationship.getSource().getName().equals(cls.getName())) {
                ...
                String newSource =
                        " ("
                                + curRelationship.getTypeString()
                                + ")"
                                + ": "
                                + curRelationship.getSource().getName()
                                + " -> "
                                + curRelationship.getDestination().getName();
        ...
        finalString =
                finalString + "  Relationships:" + sourceRelationships + destRelationships + "\n";
        return finalString;
    }
    ...
    public String listMethods(ClassObject cls) throws Exception {
        ...
        String finalString =
                "Available Methods:\n1. "
                        ...
                        + listParams((Method) methodList.get(0));
        AttributeInterface attr;
    ...
    private String listParams(Method mthd) {
        ...
        if (mthd.getParamList().size() > 0) {
            str =
                    str
                            + mthd.getParamList().get(0).getName()
                            + ": "
                            + mthd.getParamList().get(0).getType();
            for (int i = 1; i < mthd.getParamList().size(); i++) {
                str =
                        str
                                + ", "
                                + mthd.getParamList().get(i).getName()
                                + ": "
                                + mthd.getParamList().get(i).getType();
            }
        }
        return str = str + ")";
    }
    ...
    /**
     * Checks if method with methodName exists If yes, list the arities of any methods with
     * methodName
     *
    ...
    public String listMethodArities(ClassObject cls, String methodName) throws Exception {
        ...
        if (methodList.size() == 0) {
            throw new Exception(
                    "No methods with name " + methodName + " exist in " + cls.getName());
        }
     ...
     *
     * <p>A string is valid as a class name assuming: 1. It isn't blank or null 2. It begins with a
     * letter or underscore 3. It contains only letters, numbers, underscores, and/or dollar signs
    ...
    public void isValidClassName(String className) throws Exception {
        ...
        if (!Character.isLetter(className.charAt(0)) && className.charAt(0) != '_') {
            throw new Exception(
                    "Name " + className + " is invalid. First character must be a letter or '_'");
        }
        ...
        for (int i = 0; i < className.length(); i++) {
            if (!Character.isLetterOrDigit(className.charAt(i)) && className.charAt(i) != '_') {
                throw new Exception(
                        "Name "
                                + className
                                + " is invalid. Name can only contain alphanumerics, '_', or '$'");
            }
        }
    ...
    /**
     * Method A: Creates a deep copy of the model by converting it into a String and then
     * reconstructing a model from that string
     *
