...
public class ClassObject implements ClassObjectInterface {
    ...
    public ClassObject(ClassObject o) {
        ...
        for (AttributeInterface m : o.attrMap.get("Method")) {
            Method mo = (Method) m;
            attrMap.get("Method")
                    .add(new Method(m.getName(), mo.getReturnType(), mo.getParamList()));
        }
    }
            ...
            if (methodName.equals(attrMap.get("Method").get(index).getName())) {
                ...
                if (types.length != 0) {
                    ...
                    for (String curType : types) {
                        if (!curType.equalsIgnoreCase(
                                activeMethod.getParamList().get(paramIndex).getType())) {
                            continue;
                        }
                        paramIndex++;
                    }
    ...
    public Method fetchMethod(String methodName) throws Exception {
        ...
        while (index < attrMap.get("Method").size()) {
            Method activeMethod = (Method) attrMap.get("Method").get(index);
            if (methodName.equals(activeMethod.getName())
                    && activeMethod.getParamList().size() == 0) {
                return activeMethod;
            }
            index++;
        }
    ...
    public Method fetchMethod(String methodName, LinkedHashMap<String, String> paramMap)
            throws Exception {
