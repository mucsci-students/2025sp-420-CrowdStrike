...
public class ClassBox extends JLayeredPane {
    ...
    public ClassBox(ClassObject classObject, GUIController controller) {
        ...
        MouseAdapter commonMouseAdapter =
                new MouseAdapter() {
                    @Override
    ...
    public Dimension getPreferredSize() {
        int widthPadding = 35;
        int heightPadding =
                Math.max(fieldsList.getModel().getSize(), methodsList.getModel().getSize()) * 15
                        + 35;
        ...
        int totalWidth =
                Math.max(classLabelSize.width, Math.max(fieldsSize.width, methodsSize.width))
                        + widthPadding;
        int totalHeight =
                classLabelSize.height + fieldsSize.height + methodsSize.height + heightPadding;
    ...
    private void updateMethodsScrollPaneSize() {
        ...
        methodsScrollPane.setPreferredSize(
                new Dimension(listPref.width + 35, listPref.height + 35));
    }
    ...
    public String displayMethod(Method m) {
        ...
        for (int i = 0; i < m.getParamList().size(); i++) {
            sig.append(
                    m.getParamList().get(i).getName() + ": " + m.getParamList().get(i).getType());
